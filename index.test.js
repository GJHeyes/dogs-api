...
describe("Endpoints", () => {
  ...
  describe("POST /dogs", () => {
    it("should create a new dog with the request data", async () => {
    ...
    it("should match the id in the databse", async () => {
      const response = await request(app).post("/dogs").send(testDogData);
      const dog = await Dog.findOne({ where: { id: response.body.id } });
      expect(response.body.id).toEqual(dog.id);
    });
  });
  describe("DELETE /dogs/:id", () => {
    it("should delete a dog based on the id params", async () => {
